...
Install the shadcn/ui components

```sh
npx shadcn@latest add breadcrumb card checkbox dropdown-menu label popover select separator table textarea sonner skeleton carousel
```
...
It safely combines classes, handles conditions, resolves conflicts, and makes component styling predictable and maintainable. It's essential for any project using Tailwind CSS with conditional styling!
...
npx prisma migrate dev --name init creates a new migration for your database schema changes and applies it, while npx prisma db push directly updates the database schema without creating a migration. In the context of databases, a migration is set of operations, that modifies the database schema, helping it evolve over time while preserving existing data.
...
Generate the client (creates the code)
...
Execute the JavaScript code and create the data in your database
...
Wrap the html tag in layout.tsx with the ClerkProvider tag
...
- lots of code just to access input values
